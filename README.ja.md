...
- `Settings` -> `Streaming` -> `YouTube relay` から任意で YouTube Live relay を設定し、配信コメントを既存チャットへ取り込める
...
- `NEXT_PUBLIC_GOOGLE_CLIENT_ID`
  - YouTube Live relay 用の Google OAuth Web client ID
- `BASE_PATH`
...
4. 必要に応じて `Settings` から model、voice、system prompt などの基本設定を調整します。
5. 配信オプションを使う場合は `Settings` -> `Streaming` -> `YouTube relay` を開き、`NEXT_PUBLIC_GOOGLE_CLIENT_ID` または画面入力の client ID を使って Google でログインし、対象配信を選び、relay を有効化します。Gemini の自動返答は auto-reply を別途 ON にしたときだけ動作します。
...
- YouTube relay では、Google OAuth client ID と短命の YouTube access token を、サインアウトまたは期限切れまでブラウザの local storage に保存します。
